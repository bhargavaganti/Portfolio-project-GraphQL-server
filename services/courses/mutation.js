const { courses: sql } = require('../utils/sql');
const f = require('../utils/func');
const e = require('../utils/err-messages');

const subscription = async (pubsub, mutation, data) => pubsub.publish(`courses`, { courses: { mutation, data } });
const validation = async (c, name) => ((await f.query(c, sql.nameValidate, name))[0]).count > 0

module.exports = {
    async create(parent, { data }, { pubsub, connection: con }, info) {
        insertData = [
            data.name, 
            data.alt_name, 
            data.description,
        ...
        if (await validation(con, data.name)) {
            return new Error(e.nameExists);
        }
        const result = await f.query(con, sql.create, insertData);
    ...
    async update(parent, { id, data }, { pubsub, connection: con }, info) {
