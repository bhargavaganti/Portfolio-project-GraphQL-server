const { sources: sql } = require('../utils/sql');
const f = require('../utils/func');
const e = require('../utils/err-messages');

const subscription = async (pubsub, mutation, data) => pubsub.publish(`sources`, { sources: { mutation, data } });
const validation = async (c, name) => ((await f.query(c, sql.nameValidate, name))[0]).count > 0

module.exports = {
    async create(parent, { name, url }, { pubsub, connection: con }, info) {
        if (await validation(con, name)) {
            return new Error(e.nameExists);
        }
        const result = await f.query(con, sql.create, [name, url]);
    ...
    async update(parent, { id, name, url }, { pubsub, connection: con }, info) {
        if (await validation(con, name)) {
            return new Error(e.nameExists);
        }
